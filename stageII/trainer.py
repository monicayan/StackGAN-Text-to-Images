'''
Modified by: Tao Tao， Xiaoxiao Yan
Data: Dec 19, 2018
'''
...
class CondGANTrainer(object):
    ...
    def build_placeholder(self):
        ...
        self.hr_wrong_images = tf.placeholder(tf.float32, [self.batch_size] + self.hr_image_shape, name='wrong_hr_images')
        self.embeddings = tf.placeholder(tf.float32, [self.batch_size] + self.dataset.embedding_shape, name='conditional_embeddings')
    ...
    def sampler(self):
        ...
        with tf.variable_scope("hr_g_net", reuse=True):
            ...
            self.hr_fake_images = self.model.hr_get_generator(self.fake_images, hr_c)
    ...
    def epoch_sum_images(self, sess, n):
        ...
        scipy.misc.imsave('%s/lr_fake_train.jpg' % (self.log_dir), gen_samples[0])
        scipy.misc.imsave('%s/lr_fake_test.jpg' % (self.log_dir), gen_samples[1])
        scipy.misc.imsave('%s/hr_fake_train.jpg' % (self.log_dir), hr_gen_samples[0])
        scipy.misc.imsave('%s/hr_fake_test.jpg' % (self.log_dir), hr_gen_samples[1])
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % cfg.GPU_ID):
                ...
                saver = tf.train.Saver(tf.all_variables(), keep_checkpoint_every_n_hours=5)
    ...
    def drawCaption(self, img, caption):
        ...
        # get a font
        fnt = ImageFont.truetype('Pillow/Tests/fonts/Raleway-Regular.ttf', 50)
